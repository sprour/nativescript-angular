...
export class NativeScriptAnimationPlayer implements AnimationPlayer {
    ...
    constructor(element: Node, keyframes: AnimationKeyframe[], duration: number, delay: number, easing: string) {
        ...
        animationInfo.curve = easing ? NativeScriptAnimationPlayer.animationTimingFunctionConverter(easing) : enums.AnimationCurve.ease;
        ...
        for (let style of keyframe.styles.styles) {
            for (let substyle in style) {
                ...
                if (property) {
                    ...
                    NativeScriptAnimationPlayer.parseTransform(<string>value, animationInfo);
    ...
    play(): void {
        if (this.animation) {
            if (this.keyframe) {
                ...
                for (let style of this.keyframe.styles.styles) {
                    for (let substyle in style) {
                        let value = style[substyle];
                        let property = styleProperty.getPropertyByCssName(substyle);
                        if (property) {
